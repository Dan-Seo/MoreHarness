...
- **TDD mode (06) feeds a different `allowed` and `effective_forbidden` into this adjudication for each phase.** The adjudication procedure itself is exactly the one here; the per-phase composition is canonical in 06.
