...
*이 문서는 [`docs/07-CONTEXT-SELECTION.md`](../07-CONTEXT-SELECTION.md)의 번역이다. 둘이 어긋나면 영문이 canonical이다.*
...
L0~L4와 L8은 고정 확보한다. L5~L7이 남은 예산을 채우고, 예산이 모자라면 낮은 우선순위부터 탈락한다. L7의 후보는 `.harness/knowledge/`의 카드 전부다 — 정밀한 선별은 learn(M8)의 일이다.
...
trusted 구획의 원문은 **항상 메인 저장소에서** 읽는다. untrusted 구획(문서 슬라이스·파일·repo map)은 **워크스페이스에서** 읽는다 — upstream이 만든 것이 거기 있다 (05). 워크스페이스의 spec 사본을 읽으면 앞선 task의 agent가 고친 것을 trusted로 주입하는 경로가 생긴다.
...
이 키는 `.harness/config.yaml` 최상위의 `context`이며 **07이 소유한다** (03의 config 참조).

- L0~L4와 L8을 먼저 확보한다. L8은 몇 줄짜리 사실이라 예산에 거의 들지 않는다. L0가 예산을 넘으면 그것은 constitution이 너무 크다는 신호이며, 자르지 않고 경고한다.
- 남은 예산을 L5 → L6 → L7 순으로 채운다.
- 예산이 모자라면 **낮은 우선순위 계층부터 통째로 탈락**시킨다. 계층 내부를 임의로 자르지 않는다. 잘린 문서 조각은 오해를 낳는다.
- `slice_max_tokens`를 넘는 슬라이스는 자르지 않고 **통째로 탈락**시키고 manifest에 사유를 남긴다. 상한이 적용되는 것은 슬라이스 계층(L2·L3·L5·L6·L7)뿐이다 — L0·L1·L4·L8은 자르지도 탈락시키지도 않는다.
- 토큰 수 추정은 어댑터가 usage를 보고하지 않아도 계산 가능한 로컬 추정치를 쓴다 — **문자 수를 4로 나눠 올림한 값**이다. 정확도보다 **재현성**이 중요하다.
