...
*이 문서는 [`docs/10-FAILURE-RECOVERY.md`](../10-FAILURE-RECOVERY.md)의 번역이다. 둘이 어긋나면 영문이 canonical이다.*
...
이 순서가 불변이다. 커밋된 이벤트는 손실되지 않으며, 끝나지 않은 tail은 버릴 수 있다.

| 죽은 지점 | 결과 |
|---|---|
| 1 이전 | 아무 일도 없었던 것과 같다 |
| 1과 2 사이 | 개행으로 끝나지 않은 tail은 미완성으로 읽기에서 무시한다. 다음 append 전에 직전 개행까지 잘라내고 fsync한다. 그 이벤트는 없었던 것이 된다 |
| 2와 3 사이 | journal 이 앞서 있다. 재개 시 `last_applied_seq` 이후를 재생하면 복원된다 |
| 3 이후 | 정상 |

- `Journal.read`는 개행으로 끝난 바이트만 완전한 것으로 보므로 찢긴 UTF-8 tail을 decode하지 않고 버린다. append 전에 writer가 그 tail을 복구하며, 앞선 완전한 이벤트는 바이트 단위로 그대로 둔다.
- 개행으로 끝난 잘못된 줄은 찢긴 tail이 아니다. read와 append가 journal corruption 오류를 올려 `doctor`가 보고하게 하며, 불변 이벤트를 조용히 다시 쓰지 않는다.
