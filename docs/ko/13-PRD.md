...
저장소 루트의 [`PRD.md`](../../PRD.md)가 두 모양 모두에서 이것을 정리한다. 채워지지 않은
템플릿 블록을 아직 달고 있으면 그 저장소는 프레임워크이고, 채워지는 순간 그 저장소는
거기 적힌 프로젝트다. `AGENTS.md`와 `CLAUDE.md`가 코딩 에이전트를 무엇보다 먼저 그 파일로
보낸다. **이 문서는 프레임워크 자신의 제품 요구사항이지 그 파일의 대체물이 아니다.**
