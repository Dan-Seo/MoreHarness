...
fork는 두 가지 모양 중 하나로 도착하고, 그 차이가 프로젝트가 어디에 사는지를 결정한다.

- **설치하고 하네스를 프로젝트로 겨눈다.** 하네스는 별도의 클론으로 남고 `harness init`은
  프로젝트 자기 저장소 안에서 돈다. 저장소 두 개가 관여하며, 이 둘을 혼동하는 것이 가장
  흔한 첫 실수다.
- **이 저장소를 fork해서 그 안에서 만든다.** fork한 저장소가 곧 프로젝트가 되고 하네스를
  함께 지고 간다. 저장소는 하나지만 위험이 옮겨 간다. 그 저장소를 읽는 코딩 에이전트가
  사방의 프레임워크 문서를 보고 **프레임워크를 제품으로 착각한다.**

저장소 루트의 [`PRD.md`](../../PRD.md)가 그 저장소가 어떤 프로젝트인지를 정한다. 채워지지
않은 템플릿 블록을 아직 달고 있으면 그 저장소는 프레임워크이고, 채워지는 순간 그 저장소는
거기 적힌 프로젝트다. **이 문서는 프레임워크 자신의 제품 요구사항이지 그 파일의 대체물이
아니다.**

`init`은 두 모양 모두에서 그 파일을 쓴다. 하지만 에이전트를 그 파일로 보내는 것은 다르다.
fork한 저장소는 [`AGENTS.md`](../../AGENTS.md)와 [`CLAUDE.md`](../../CLAUDE.md)를 함께
지고 가고, 이 둘이 템플릿 블록을 보고 갈라지며 무엇보다 먼저 읽힌다. 하네스를 겨누기만 한
저장소에는 둘 다 없으므로 그 파일을 읽을 이유를 알려 주는 것이 아무것도 없다. 그 저장소의
주인이 자기 에이전트가 이미 읽는 지침 파일에 그 포인터를 넣는다.

아래 각 단계는 자기 확인 방법을 함께 가진다.
...
| 2c | 에이전트 지침 파일이 `PRD.md`를 가리키게 한다 — fork는 이미 되어 있다 | `AGENTS.md`·`CLAUDE.md` 등 에이전트가 읽는 파일 | 이 저장소가 무엇을 만드느냐는 물음에 에이전트가 프로젝트를 답한다 |
