...
development:                  # if not declared, mode: standard
...
### What `development` determines

| `mode` | the shape of one attempt |
|---|---|
| `standard` | one dispatch. The default, and where a task that does not declare `development` belongs |
| `tdd` | test-author → red gate → implementation → green gate. Two dispatches |

`tdd` requires `test_paths` and `implementation_paths`. Both use the glob dialect of 05 and
are relative to the repository root. Which list is allowed and which is forbidden in each
phase, and what each gate requires, is canonical in 06.
...
| `tdd_phase_completed` | a TDD phase boundary | `phase`, `base`, `ok`, `detail` |
...
`ac_baseline_executed` also records the optional field `criterion_index` so that a split
baseline can be restored in its original declaration order on resume. If an older journal
...
that does not change state.** There is no separate event for the green gate — that result is
the `differential` of `ac_post_executed`.
  ...
  .harness/                       # the harness's own control-plane
    ...
    runs/<run-id>/
      ...
      tasks/T-003/
        ...
        prompt.md
        prompt.test-author.md  prompt.implementation.md   # TDD phase prompts — 06
